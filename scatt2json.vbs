...
WScript.echo "Scatt Professional to JSON converter"
...
if doc Is Nothing then
	WScript.echo "Error: Scatt Professional was not installed properly."
	return
end if
...
Sub ProcessDocument(doc,filename)
	...
	if Not doc.Valid then
		WScript.echo "Error: File not found or invalid file format."
		return
	end if
	...
	if AllShots.Count = 0 then
		WScript.echo "Error: No match shots"
		return
	end if

	Dim outputFilename
	outputFilename = fso.GetBaseName(filename)
	Dim d
	...
	Dim outputFile
	On Error Resume Next
	Set outputFile = fso.CreateTextFile(outputFilename & ".json" , True)
	if Err.Number <> 0 then
	 	WScript.echo "Error: Can't create output file - " & Err.Description
		return
	end if
